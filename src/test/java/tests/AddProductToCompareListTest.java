...
public class AddProductToCompareListTest extends TestBase{
	...
	public void UserCanCompareProducts() throws InterruptedException {
		...
		Assert.assertTrue(detailsObject.productname.getText().contains(firstProductName));
		detailsObject.AddProductToCompare();

		searchObject.productSearchUsingAutoSuggest("Asus");
		Assert.assertTrue(detailsObject.productname.getText().contains(secondProductName));
		detailsObject.AddProductToCompare();
		Thread.sleep(1000);
